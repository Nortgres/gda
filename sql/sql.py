...
'''cursor.execute('SELECT DISTINCT shop FROM products') #SELECT DISTINCT - отбрасывает дубли значений
res = cursor.fetchall()
for i in res:
    print(i)'''

'''cursor.execute('SELECT count(DISTINCT shop) shop FROM products')
res = cursor.fetchall()
for i in res:
    print(i)'''


'''cursor.execute('SELECT MIN(price) shop FROM products')
res = cursor.fetchall()
for i in res:
    print(i)

cursor.execute('SELECT MAX(price) shop FROM products')
res = cursor.fetchall()
for i in res:
    print(i)

cursor.execute('SELECT AVG(price) shop FROM products') #AVG среднее значение
res = cursor.fetchall()
for i in res:
    print(i)'''
